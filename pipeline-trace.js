...
async function main() {
    ...
    const rootSpan = tracer.startSpan(job, {
        ...
        steps = [{
            ...
            },
            {
                ...
                run: async () => {
                    ...
                    try {
                        execSync('npm run test --if-present', {
                            stdio: 'inherit'
                        });
                    } catch (e) {}
                },
            },
            {
                ...
                run: async () => {
                    log('Zipping release...');
                    execSync('zip release.zip . -r -x "node_modules/*" -x ".git/*"', {
                        stdio: 'inherit'
                    });
                },
            }
    ...
    for (const step of steps) {
        await otel.context.with(ctx, async () => {
            ...
            try {
                ...
                } = await timeAsync(step.name, async () => await step.run());
                duration = stepDuration;
                span.setStatus({
